...
describe JSONAPI::FetchData::Middleware do
  ...
  let(:middleware) { described_class.new app, :page, :fields }
  ...
  let(:request) { Rack::MockRequest.env_for('https://example.org', params: query_string) }
  ...
  it 'should parse query string value only for those passed in options' do
    ...
    expect_any_instance_of(JSONAPI::FetchData::Parameters::Parsers::Paginate).to receive(:parse)
    expect_any_instance_of(JSONAPI::FetchData::Parameters::Parsers::Filter).to_not receive(:parse)
